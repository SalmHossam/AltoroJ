...
<%= StringEscapeUtils.escapeHtml4(request.getParameter("startDate")) %>
...
<div id="wrapper" style="width: 99%;">
    ...
    <td valign="top" colspan="3" class="bb">
		...
		<form id="Form1" name="Form1" method="post" action="showTransactions" onsubmit="return confirminput(this);">
			<table border="0" style="padding-bottom:10px;">
				<tr>
					<td valign="top">After</td>
					<td><input id="startDate" name="startDate" type="text" value="<%=request.getParameter("startDate")==null ? "" : StringEscapeUtils.escapeHtml4(request.getParameter("startDate"))%>"/><br /><span class="credit">yyyy-mm-dd</span></td>
					<td valign="top">Before</td>
					<td><input name="endDate" id="endDate" type="text" value="<%=request.getParameter("endDate")==null ? "" : StringEscapeUtils.escapeHtml4(request.getParameter("endDate"))%>"/><br /><span class="credit">yyyy-mm-dd</span></td>
			...
			<table cellspacing="0" cellpadding="3" rules="all" border="1" id="_ctl0__ctl0_Content_Main_MyTransactions" style="width:100%;border-collapse:collapse;">
				...
				<%
                    for (int i = 0; i < transactions.length && i < 100; i++) {
                        Transaction transaction = transactions[i];
                %>
				<tr>
					<td><%= StringEscapeUtils.escapeHtml4(String.valueOf(transaction.getTransactionId())) %></td>
					<td><%= StringEscapeUtils.escapeHtml4(transaction.getDate().toString()) %></td>
					<td><%= StringEscapeUtils.escapeHtml4(String.valueOf(transaction.getAccountId())) %></td>
					<td><%= StringEscapeUtils.escapeHtml4(String.valueOf(transaction.getTransactionType())) %></td>
					<td align="right"><%= StringEscapeUtils.escapeHtml4(String.valueOf(transaction.getAmount())) %></td>
				</tr>				
                <%
                    }
                %>
